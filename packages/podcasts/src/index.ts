...
import { compileArticle, type CompiledArticle } from "./lib.js";
...
async function fetchUpdatedArticles(
  articlesPath: string
): Promise<Array<CompiledArticle>> {
  const podcastsCache = await loadPodcastsCache();
  const updatedArticles: Array<CompiledArticle> = [];
  ...
  await fs.writeFile(
    path.resolve(".podcasts/cache.json"),
...
const buildSelectArticlesPrompt = (
  articles: Array<CompiledArticle>
 ...
 */
async function generate(articlesDir: string, outDir: string) {
  const updatedArticles = await oraPromise(
    fetchUpdatedArticles(articlesDir),
  ...
  const selectedArticles = new Set(
    (await prompts(buildSelectArticlesPrompt(updatedArticles))).articles
  );
  ...
  await Promise.all(
    updatedArticles
      ...
      .map(
        async (article) =>
          await oraPromise(
            generatePodcast(openai, article, outDir),
            `${chalk.blue("generating podcast for")} ${chalk.blueBright(article.frontmatter.slug)}`
          )
      )
  );
}
...
cli
  ...
  .action(
    ({
      dotenv: useDotenv,
      articlesDir = path.resolve("src/content/articles/"),
      outDir = path.resolve("public/assets/podcasts"),
    }) => {
      if (useDotenv) {
        dotenv.config();
      }
